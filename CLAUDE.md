...
15. **开发工具必须先立 spec、再写测试、最后写实现**（2026-09-12 要求）。
    ...
    反面教材：`docs/specs/markdown-cleaner.md` 是**回写**的——当时先写完实现才补测试，
    结果清理规则里最容易错的部分（`snake_case` 的下划线、表格分隔行与水平线之争、
    标点只在 CJK 字符之后才转全角）全是被测试逼出来的，而不是设计时想清楚的。
    工具页的接线清单（数据源、路由、i18n、预渲染）见该 spec 第 8 节。
